...
Users can also start [JupyterHub sessions](jupyterhub) to get
...
<td>high (default)</td>
<td>352</td>
<td>28 days<sup>2</sup></td>
<td>132 GB (eml-sm2 nodes) or 768 GB (eml-sm3 nodes, requested with "-C mem768g" [formerly "-C bigmem"] <sup>4</sup>)</td>
<td>56 (eml-sm2 nodes) or 32 (eml-sm3 nodes)<sup>3</sup></td>
</tr>
<tr class="even">
<td>low<sup>1</sup></td>
<td>256</td>
...
\[1\] See [How to Submit Jobs to the Low Partition](#low).

\[2\] See [Submitting Long Jobs](#long) for jobs you expect to take more
than three days.
...
[Submitting Multi-node Jobs](#multi-node) or [Submitting MPI Jobs](#MPI)
or [Submitting MATLAB Parallel Server Jobs](#parallel-server).

\[4\] See [Submitting Large Memory Jobs](#bigmem) for jobs needing more
than 25 GB memory.

\[5\] See [Submitting GPU Jobs](#gpu) for jobs using the GPU.

## Basic Slurm Usage

### Quick Start Guide

Our sister facility, the Statistical Computing Facility, has a [quick
start
guide](https://statistics.berkeley.edu/computing/servers/cluster#jump%5Baccordion%5D%5B1%5D%5B1%5D)
...
### <span id="long"></span>Submitting Long Jobs and Setting Job Time Limits
...
### <span id="bigmem"></span>Submitting Large Memory Jobs
...
above and discussed [further here](memory).
...
### <span id="gpu"></span>Submitting GPU Jobs

To submit a job to the GPU node, you must include either the
'--partition=gpu' or '-p gpu' flag, as well as the "--gres=gpu:1" flag.
For example:
...
```bash
theil:~/Desktop$ srun --pty --jobid=380 /bin/bash
```
...
```bash
theil:~/Desktop$ sacct  --starttime=2021-04-01 --endtime=2021-04-30 \
...
-   \`shist\`: provides information about completed (including failed)
    jobs
-   \`sassoc\`: gives information about user access to cluster
    partitions
...
omp\_set\_num\_threads() function in the RhpcBLASctl package, again
making use of the SLURM\_CPUS\_PER\_TASK environment variable.\]
...
for Stata). IMPORTANT: Do not request more than 8 cores for a Stata job.
If you request eight cores (--cpus-per-task=8) or fewer, you are all
set.
...
future's future\_lapply, foreach, mclapply, parLapply).
...
IMPORTANT: Your R, Python, or any other code won't be able to use more
cores than the number of total cores requested (4 in this case). You can
use the SLURM\_CPUS\_PER\_TASK environment variable to programmatically
...
### <span id="multi-node"></span>Submitting Multi-node Jobs
...
### <span id="MPI"></span>Submitting MPI Jobs
...
Your program should then make use of the SLURM\_ARRAY\_TASK\_ID
...
{0,1,2,5,7}). You could, for example, read SLURM\_ARRAY\_TASK\_ID into
your R, Python, MATLAB, or C code.

Here's a concrete example where it's sufficient to use
SLURM\_ARRAY\_TASK\_ID to distinguish different input files if you need
...
SLURM\_PROCID environment variable, which will be distinct (and have
